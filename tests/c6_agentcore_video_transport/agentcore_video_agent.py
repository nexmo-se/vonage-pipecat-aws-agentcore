#!/usr/bin/env python3
"""
C6 AgentCore Runtime deploy artifact.

BedrockAgentCoreApp hosting VonageVideoConnectorTransport + Pipecat pipeline.
Non-blocking @app.entrypoint spawns long-running pipelines via asyncio.create_task.

Production: session_id and token come from AgentCore invoke payload/context
(per call) — not from static env vars.

Actions (HTTP /invocations payload):
  {"action": "network_probe"}
  {"action": "join", "session_id": "...", "token": "...", "mode": "echo"|"nova_sonic"}
  {"action": "leave"}
  {"action": "status"}
"""
